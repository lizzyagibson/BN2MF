...
sim_old <- rbind(sim_dist %>% mutate(data = "Distinct"),
  ...
  mutate(std_sim = map(sim, function(x) apply(x, 2, function(x) x/sd(x))), #standardize
         std_10_sim = map(std_sim, function(x) x*10)) %>% # scale sd up by 10
  pivot_longer(cols = c(sim, std_sim, std_10_sim),
               values_to = "sim",
               names_to = "sim_type")

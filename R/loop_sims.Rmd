...
```{r}
sim_dist <- sim_dist %>% rename(true_patterns = patterns, true_scores = scores) %>% select(-chem, -noise)
...
```{r}
sim_cor  <- sim_cor %>% rename(true_patterns = patterns, true_scores = scores) %>% select(-chem, -noise)
...
```{r} 
sim_over <- sim_over %>% rename(true_patterns = patterns, true_scores = scores) %>% select(-chem, -noise)
...
```{r}
sim_dist_test <- sim_dist_test %>% select(-noise, -chem)
...
```{r}
sim_cor_test <- sim_cor_test %>% select(-noise, -chem)
...
```{r}
sim_over_test <- sim_over_test %>% select(-noise, -chem)

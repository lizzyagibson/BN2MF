...
**In fa.stats(r = r, f = f, phi = phi, n.obs = n.obs, np.obs = np.obs,...: The estimated weights for the factor scores are probably incorrect.  Try a different factor score estimation method.**  
...
sd_dist_out <- fa_out_dist %>% 
  ...
  mutate(sd_patterns = map2(sd_loading, sd_patterns, add_row_true),
         sd_scores   = map2(sd_fa_score, sd_scores, add_column_true),
...
```{r}
over_score_s <- summary(over_norm_fa_score) %>% as.matrix() %>% t() %>% 
  as_tibble() %>% mutate(Norm = "Relative Score Error",
                         Simulation = "Overlapping",
                         Standardized = "Yes") %>% 
  select(Simulation, Norm, Standardized, everything())

cor_score_s <- summary(cor_norm_fa_score) %>% as.matrix() %>% t() %>% 
  as_tibble() %>% mutate(Norm = "Relative Score Error",
                         Simulation = "Correlated",
                         Standardized = "Yes") %>% 
  select(Simulation, Norm, Standardized, everything())

dist_score_s <- summary(dist_norm_fa_score) %>% as.matrix() %>% t() %>% 
  as_tibble() %>% mutate(Norm = "Relative Score Error",
                         Simulation = "Distinct",
                         Standardized = "Yes") %>% 
  select(Simulation, Norm, Standardized, everything())

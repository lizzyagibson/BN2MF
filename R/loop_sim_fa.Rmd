...
get_loadings <- function (sim) {
  matrix(factanal(sim, 4, rotation="promax")$loadings, ncol = 4)
}
...
dist_norm <- fa_out_dist %>% 
  mutate(sd_patterns = map(patterns, ~standardize(.x)),
         sd_loadings = map(loadings, ~standardize(t(.x)))) %>% 

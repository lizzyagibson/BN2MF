...
sim_dist <- sim_dist0 %>% 
 mutate(sim = map2(seed, chem, add_noise),
        sim_sd = map(sim, function(x) scale(x, center = FALSE, scale = apply(x, 2, sd))))
```  

```{r}
dist <- sim_dist[1,5][[1]][[1]]
summary(apply(dist, 2, var))
...
```{r}
over <- sim_over[1,5][[1]][[1]]
summary(apply(over, 2, var))

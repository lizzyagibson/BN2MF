...
dist_patterns <- sd_npb_1 %>% 
  mutate(sd_patterns = map(sd_patterns, function(x) as_tibble(t(x)))) %>% 
  select(-sd_eh) %>% 
  unnest(cols = c(sd_patterns))

colnames(dist_patterns)[2:51] <- labels

labelled_dist_patterns <- dist_patterns %>%
  mutate(pattern1 = ifelse(pcb146 > 0.03 & pcb28 > bpa, 1,
                          ifelse(BDE17 > 0.03, 2,
                                ifelse(MECPP > 0.03, 3,
                                       ifelse(tcs > 0.03, 4, 5))))) %>%
  select(seed, pattern1, everything()) %>%
...
```{r, fig.width=8}
dist_nbp <- sd_npb_1 %>% 
  mutate(sd_eh = map(sd_eh, function(x) as_tibble(t(x)))) %>% 
  select(-sd_patterns) %>% 
  unnest(cols = c(sd_eh))

colnames(dist_nbp)[2:51] <- labels

labelled_dist_nbp <- dist_nbp %>%
  mutate(pattern1 = ifelse(pcb146 > 0.03 & pcb28 > bpa, 1,
                          ifelse(BDE17 > 0.03, 2,
                                ifelse(MECPP > 0.03, 3,
                                       ifelse(tcs > 0.03, 4, 5))))) %>%
...
colnames(over_patterns)[2:51] <- labels

labelled_over_patterns <- over_patterns %>%
  mutate(pattern1 = ifelse(pcb146 > 0.02 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.02, 2,
                                  ifelse(MECPP > 0.02, 3,
                                         ifelse(tcs > 0.02, 4, 5))))) %>%
  select(seed, pattern1, everything()) %>%
...
colnames(over_nbp)[2:51] <- labels

labelled_over_nbp <- over_nbp %>%
  mutate(pattern1 = ifelse(pcb146 > 0.03 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.03, 2,
                                  ifelse(MECPP > 0.03, 3,
                                         ifelse(tcs > 0.03, 4, 5))))) %>%
...
colnames(cor_patterns)[2:51] <- labels

labelled_cor_patterns <- cor_patterns %>%
  mutate(pattern1 = ifelse(pcb146 > 0.02 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.02, 2,
                                  ifelse(MECPP > 0.02, 3,
                                         ifelse(tcs > 0.02, 4, 5))))) %>%
...
colnames(cor_nbp)[2:51] <- labels

labelled_cor_nbp <- cor_nbp %>%
  mutate(pattern1 = ifelse(pcb146 > 0.005 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.005 , 2,
                                  ifelse(MECPP > 0.005 , 3,
                                         ifelse(tcs > 0.005, 4, 5))))) %>%
  select(seed, pattern1, everything()) %>%
  arrange(seed, pattern1) %>%
  mutate(pattern1 = fct_inorder(as.factor(pattern1)),
         pattern1 = as.integer(pattern1))
...
```{r, fig.width=6}
dist_fa <- fa_sd_1 %>% 
  mutate(sd_loading = map(sd_loading, function(x) as_tibble(t(x)))) %>% 
  select(-sd_patterns) %>% 
  unnest(cols = c(sd_loading))

colnames(dist_fa)[2:51] <- labels

labelled_dist_fa <- dist_fa %>%
  mutate(pattern1 = ifelse(pcb146 > 0.03 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.03, 2,
                                  ifelse(MECPP > 0.03, 3,
                                         ifelse(tcs > 0.03, 4, 5))))) %>%
...
labelled_dist_fa %>%
  ...
  facet_wrap(.~pattern1) + theme_bw() +
  theme(legend.position = "none",
        strip.background = element_blank()) + labs(title= "FA Loadings, Distinct Patterns", x = "", y = "Normalized Loadings")
```

```{r, fig.width=6}
over_patterns <- fa_sd_2 %>% 
  mutate(sd_patterns = map(sd_patterns, function(x) as_tibble(t(x)))) %>% 
  select(-sd_loading) %>% 
  unnest(cols = c(sd_patterns))
...
colnames(over_fa)[2:51] <- labels

labelled_over_fa <- over_fa %>%
  mutate(pattern1 = ifelse(pcb146 > 0.01 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.015, 2,
                                  ifelse(MECPP > 0.015, 3,
                                         ifelse(tcs > 0.015, 4, 5))))) %>%
  select(seed, pattern1, everything()) %>%
  arrange(seed, pattern1) %>%
  mutate(pattern1 = fct_inorder(as.factor(pattern1)),
         pattern1 = as.integer(pattern1))
...
labelled_over_fa %>%
  ...
  facet_wrap(.~pattern1) + theme_bw() +
  theme(legend.position = "none",
        strip.background = element_blank()) + labs(title= "FA Loadings, Overlapping Patterns", x = "", y = "Normalized Loadings")
```

```{r, fig.width=6}
cor_patterns <- fa_sd_3 %>% 
  mutate(sd_patterns = map(sd_patterns, function(x) as_tibble(t(x)))) %>% 
  select(-sd_loading) %>% 
  unnest(cols = c(sd_patterns))
...
colnames(cor_fa)[2:51] <- labels

labelled_cor_fa <- cor_fa %>%
  mutate(pattern1 = ifelse(pcb146 > 0.01& pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.015, 2,
                                  ifelse(MECPP > 0.015, 3,
                                         ifelse(tcs > 0.015, 4, 5))))) %>%
...
labelled_cor_fa %>%
  ...
  facet_wrap(.~pattern1) + theme_bw() +
  theme(legend.position = "none",
        strip.background = element_blank()) + labs(title= "FA Loadings, Correlated Scores", x = "", y = "Normalized Loadings")
```
...
```{r, fig.width=8}
dist_nmf <- nmf_sd_1 %>% 
  mutate(sd_coef = map(sd_coef, function(x) as_tibble(t(x)))) %>% 
  select(-sd_patterns) %>% 
  unnest(cols = c(sd_coef))

colnames(dist_nmf)[2:51] <- labels

labelled_dist_nmf <- dist_nmf %>%
  mutate(pattern1 = ifelse(pcb146 > 0.025 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.025, 2,
                                  ifelse(MECPP > 0.025, 3,
                                         ifelse(tcs > 0.025, 4, 5))))) %>%
  select(seed, pattern1, everything()) %>%
  arrange(seed, pattern1) %>%
  mutate(pattern1 = fct_inorder(as.factor(pattern1)),
         pattern1 = as.integer(pattern1))
...
colnames(over_nmf)[2:51] <- labels

labelled_over_nmf <- over_nmf %>%
  mutate(pattern1 = ifelse(pcb146 > 0.01 & BDE17 > 0.01 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.01 & MECPP > 0.01, 2,
                                  ifelse(MECPP > 0.01 & tcs > 0.01, 3,
                                         ifelse(tcs > 0.01 & pcb146 > 0.01, 4, 5))))) %>%
  select(seed, pattern1, everything()) %>%
  arrange(seed, pattern1) %>%
  mutate(pattern1 = fct_inorder(as.factor(pattern1)),
         pattern1 = as.integer(pattern1))
...
colnames(cor_nmf)[2:51] <- labels

labelled_cor_nmf <- cor_nmf %>%
  mutate(pattern1 = ifelse(pcb146 > 0.01 & BDE17 > 0.01 & pcb28 > bpa, 1,
                           ifelse(BDE17 > 0.015 & MECPP > 0.015, 2,
                                  ifelse(MECPP > 0.015 & tcs > 0.015, 3,
                                         ifelse(tcs > 0.015 & pcb146 > 0.015, 4, 5))))) %>%
  select(seed, pattern1, everything()) %>%
  arrange(seed, pattern1) %>%
  mutate(pattern1 = fct_inorder(as.factor(pattern1)),
         pattern1 = as.integer(pattern1))

...
1. Repeat steps 150 times,
    ...
    b. Run BN^2MF on bootstrapped sample
...
wa_look = plot_wa %>% 
  ...
  geom_rect(aes(xmin = v_wa_25,  xmax = v_wa_75,  ymin=0, ymax=Inf), 
            color="lightblue", linetype="dotted", alpha = 0) +
  geom_rect(aes(xmin = bs_wa_25, xmax = bs_wa_75, ymin=0, ymax=Inf), 
            color="pink", linetype="dotted", alpha = 0) +
  ...
  geom_vline(aes(xintercept = bs_ewa), linetype="dotted", color = "red") + 
  geom_vline(aes(xintercept = v_ewa),  linetype="dotted", color = "blue") + 
...
h_look = plot_h %>% 
  mutate(sim_type = fct_inorder(sim_type)) %>% 
  ggplot(aes(x = Distribution)) +
  geom_rect(aes(xmin = v_h_25,  xmax = v_h_75,  ymin=0, ymax=Inf), 
            color="lightblue", linetype="dotted", alpha = 0) +
  geom_rect(aes(xmin = bs_h_25, xmax = bs_h_75, ymin=0, ymax=Inf), 
            color="pink", linetype="dotted", alpha = 0) +
  geom_histogram(aes(y=after_stat(density),fill = Type), 
                 position = "identity", bins = 100, alpha = 0.5) +
  geom_density(aes(group = Type)) +
  ...
  facet_grid(sim_type~., scales = "free") +
  geom_vline(aes(xintercept = truth),  linetype="dotted", color = "black") +
  geom_vline(aes(xintercept = bs_eh), linetype="dotted", color = "red") + 
  geom_vline(aes(xintercept = v_eh),  linetype="dotted", color = "blue") + 
  theme(legend.position = "bottom",
        strip.background = element_rect(fill="white")) +
...
pred_look = 
  ...
  mutate(sim_type = fct_inorder(sim_type)) %>% 
  ggplot(aes(x = Distribution)) +
  geom_rect(aes(xmin = v_pred_25,  xmax = v_pred_75,  ymin=0, ymax=Inf), 
            color="lightblue", linetype="dotted", alpha = 0) +
  geom_rect(aes(xmin = bs_pred_25, xmax = bs_pred_75, ymin=0, ymax=Inf), 
            color="pink", linetype="dotted", alpha = 0) +
  geom_histogram(aes(y = after_stat(density), fill = Type), 
                 position = "identity", bins = 100, alpha = 0.5) +
  geom_density(aes(group = Type)) +
  ...
  theme_bw() + 
  geom_vline(aes(xintercept = truth),  linetype="dotted", color = "black") +
  geom_vline(aes(xintercept = bs_pred), linetype="dotted", color = "red") + 
  geom_vline(aes(xintercept = v_pred),  linetype="dotted", color = "blue") + 
  facet_grid(sim_type~., scales = "free") +
  theme(legend.position = "bottom",
        strip.background = element_rect(fill="white")) +

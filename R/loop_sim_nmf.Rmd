...
match_scores <- function (patterns, scores) {
  for (j in 1:nrow(patterns)) {
    for (i in 1:ncol(scores)) {
      if (patterns[j,2] == j && scores[1,j] == j && patterns[j,1] == i) {
        scores[1002,j] = i 
      }
    }
  } 
  scores <- scores[c(1, 1002, 2:1001),]
  scores
}

scores_matched <- full_dist %>% 
...
reorder_scores <- function (ewa_unnested) {
  ewa_reordered <- scores_matched$ewa_reordered[[2]] %>% 
    t(.) %>% as_tibble(.) %>% 
  arrange(V2) %>%

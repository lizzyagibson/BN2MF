...
cormat <- round(cor(as_tibble(sim_dist$sim[[30]]), use = "complete.obs"),2)
...
```{r, echo = FALSE, fig.height = 7}
cormat <- round(cor(as_tibble(sim_cor$sim[[5]]), use = "complete.obs"),2)
...
```{r, echo = FALSE, fig.height = 7}
cormat3 <- round(cor(as_tibble(sim_over$sim[[5]]), use = "complete.obs"),2)

melted_cormat3 <- melt(cormat3) %>% rename(Correlation = value)

ggplot(data = melted_cormat3, aes(x = Var1, y = Var2)) +
  geom_tile(aes(fill = Correlation), colour = "white") +
  scale_fill_gradient2(low = "#00BFC4", mid = "white", high = "#F8766D",
                       midpoint = 0,
                       na.value = "transparent", limits = c(-1, 1)) +
...
full_dist[50,]$eh_match[[1]]
full_dist[50,]$ewa[[1]]

for (j in 1:nrow(full_dist)) {
for (i in 1:nrow(full_dist$eh[[1]])) {
    if (full_dist[j,]$eh_match[[1]][i,1] == 1) {
        full_dist[j,]$new_ewa[[1]][,i] <- full_dist[j,]$ewa[[1]][,1]
      } else if (full_dist[j,]$eh_match[[1]][i,1] == 2) {
        full_dist[j,]$new_ewa[[1]][,i] <- full_dist[j,]$ewa[[1]][,2]
      } else if (full_dist[j,]$eh_match[[1]][i,1] == 3) {
        full_dist[j,]$new_ewa[[1]][,i] <- full_dist[j,]$ewa[[1]][,3]
      } else if (full_dist[j,]$eh_match[[1]][i,1] == 4) {
        full_dist[j,]$new_ewa[[1]][,i] <- full_dist[j,]$ewa[[1]][,4]
      } else {}
}
}

full_dist[50,]$new_ewa[[1]]
...
full_cor[50,]$eh_match[[1]]
full_cor[50,]$ewa[[1]]

for (j in 1:100) {
for (i in 1:4) {
    if (full_cor[j,]$eh_match[[1]][i,1] == 1) {
      full_cor[j,]$new_ewa[[1]][,i] <- full_cor[j,]$ewa[[1]][,1]
    } else if (full_cor[j,]$eh_match[[1]][i,1] == 2) {
      full_cor[j,]$new_ewa[[1]][,i] <- full_cor[j,]$ewa[[1]][,2]
    } else if (full_cor[j,]$eh_match[[1]][i,1] == 3) {
      full_cor[j,]$new_ewa[[1]][,i] <- full_cor[j,]$ewa[[1]][,3]
    } else if (full_cor[j,]$eh_match[[1]][i,1] == 4) {
      full_cor[j,]$new_ewa[[1]][,i] <- full_cor[j,]$ewa[[1]][,4]
    } else {}
}
}

full_cor[50,]$new_ewa[[1]]
...
```{r}
full_over <- full_join(eh_over, ewa_cor_raw, by = "seed") %>% 
    full_join(., sim_over, by = "seed") %>% 
  select(-chem) %>% 
    mutate(new_ewa = map(ewa, ~.x[NA,]))

full_over[50,]$eh_match[[1]]
full_over[50,]$ewa[[1]]

for (j in 1:100) {
for (i in 1:4) {
    if (full_over[j,]$eh_match[[1]][i,2] == 1) {
      full_over[j,]$new_ewa[[1]][,1] <- full_over[j,]$ewa[[1]][,i]
    } else if (full_over[j,]$eh_match[[1]][i,2] == 2) {
      full_over[j,]$new_ewa[[1]][,2] <- full_over[j,]$ewa[[1]][,i]
    } else if (full_over[j,]$eh_match[[1]][i,2] == 3) {
      full_over[j,]$new_ewa[[1]][,3] <- full_over[j,]$ewa[[1]][,i]
    } else if (full_over[j,]$eh_match[[1]][i,2] == 4) {
      full_over[j,]$new_ewa[[1]][,4] <- full_over[j,]$ewa[[1]][,i]
    } else {}
}
}

full_over[50,]$new_ewa[[1]]

# reorder patterns 
full_over %>% select(seed, eh_match, eh, patterns) %>% 

...
add_row_true <- function(pattern, true) {
...
add_column_true <- function(score, true) {
...
scores_ordered_dist %>% 
  mutate(patterns = map2(eh_reordered, patterns, add_row_true),
         scores = map2(ewa_reordered, scores, add_column_true)) %>% 
...
sd_dist_out <- scores_ordered_dist %>% 
  ...
  mutate(sd_patterns = map2(sd_eh, sd_patterns, add_column_true),
         sd_scores   = map2(sd_ewa, sd_scores, add_column_true)) %>%
...
dist_load <- scores_ordered_dist %>% 
  ...
  mutate(patterns = map2(eh_reordered, patterns, function(x,y) add_row_true(x,y))) %>% 
...
dist_score <- scores_ordered_dist %>% 
  ...
  mutate(scores = map2(ewa_reordered, scores, function(x,y) add_column_true(x,y))) %>% 
  mutate(norm_scores = map2(scores, ewa_reordered, 
                              ~norm(as.matrix(.x) - as.matrix(.y), "F")/norm(as.matrix(.x), "F"))) %>% 
  unnest(norm_scores) %>% 
...
```{r}
over_score_s <- summary(over_norm_ewa) %>% as.matrix() %>% t() %>% 
  as_tibble() %>% mutate(Norm = "Relative Score Error",
                         Simulation = "Overlapping",
                         Standardized = "Yes") %>% 
  select(Simulation, Norm, Standardized, everything())

cor_score_s <- summary(cor_norm_ewa) %>% as.matrix() %>% t() %>% 
  as_tibble() %>% mutate(Norm = "Relative Score Error",
                         Simulation = "Correlated",
                         Standardized = "Yes") %>% 
  select(Simulation, Norm, Standardized, everything())

dist_score_s <- summary(dist_norm_ewa) %>% as.matrix() %>% t() %>% 
  as_tibble() %>% mutate(Norm = "Relative Score Error",
                         Simulation = "Distinct",
                         Standardized = "Yes") %>% 
  select(Simulation, Norm, Standardized, everything())

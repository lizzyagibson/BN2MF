...
match_patterns_npb <- function (eh_unnested) {
  ...
  dplyr::select(pattern1, everything())
  eh_reordered
}
...
eh_dist_reorder <- eh_dist_unnest %>%
  mutate(pattern1 = ifelse(pcb146 > 0.5 & pcb28 > bpa, 1,
                          ifelse(BDE17 > 0.5, 2,
                                ifelse(MECPP > 0.5, 3,
                                       ifelse(tcs > 0.5, 4, 5))))) %>%
  dplyr::select(seed, pattern1, everything()) %>%
...
npb_scores_ordered_dist %>% 
  mutate(number_patterns = map(eh_reordered, nrow),
         symm_dist = map2(true_scores, ewa_reordered, function(x,z) symm_subspace_dist(x,z)[[1]]),
         symm_ratio = map2(true_scores, ewa_reordered, function(x,z) symm_subspace_dist(x,z)[[2]]),) %>% 
  dplyr::select(seed, number_patterns, symm_dist, symm_ratio) %>% 
  unnest(cols = c(number_patterns, symm_dist, symm_ratio)) %>% dplyr::select(symm_ratio) %>%
...
npb_dist_pred <- npb_scores_ordered_dist %>%
  ...
  as_tibble() %>% mutate(Norm = "Relative Prediction Error",
                         Simulation = "Distinct",
                         Standardized = "No") %>% 
  dplyr::select(Norm, Simulation, Standardized, everything())
...
sd_npb_dist_out <- npb_scores_ordered_dist %>% 
  ...
  dplyr::select(-eh_reordered, -ewa_reordered, -scores, -patterns)

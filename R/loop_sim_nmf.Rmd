...
pat <- order_patterns$eh_reordered[[54]]
sco <- order_patterns$ewa[[54]]
new_sco <- order_patterns$ewa_reordered[[54]]
  
for (j in 1:nrow(pat)) {
    for (i in 1:ncol(sco)) {
      if (pat[j,2] == j && sco[1,j] == j && pat[j,1] == i) {
        new_sco[1002,j] = i 
        #new_sco <- new_sco %>% (V1, 1002, everything())
      }
    }
} 

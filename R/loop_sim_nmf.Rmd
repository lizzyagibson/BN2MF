...
```{r}
apply(sim_dist[1,4][[1]][[1]], 2, summary)
apply(sim_over[1,4][[1]][[1]], 2, summary)
apply(sim_cor[1,4][[1]][[1]], 2, summary)
```
...
npb_scores_matched_dist %>% 
  ...
  unnest(cols = c(symm_ratio)) %>% dplyr::select(symm_ratio) %>%
  ggplot(aes(x = symm_ratio)) + geom_histogram(bins = 100) +
  theme_minimal() +
  labs(y = "Count", x = "Symmetric Subspace Distance Ratio")

npb_scores_matched_dist %>% 
  mutate(symm_ratio = map2(true_patterns, npbnmf_loadings, symm_subspace_dist)) %>% 
  dplyr::select(seed, symm_ratio) %>% 
  unnest(cols = c(symm_ratio)) %>% dplyr::select(symm_ratio) %>%
  ggplot(aes(x = symm_ratio)) + geom_histogram(bins = 100) +
  theme_minimal() +

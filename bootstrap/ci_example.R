...
for (boot in 1:bootstraps) {
    ...
    bs_ewa[,,boot]  = readMat(paste0("./misc/bsci_out/ewa_bs_sim_", set, "_bs_", boot, ".mat"))[[1]]
    bs_eh[,,boot]    = readMat(paste0("./misc/bsci_out/eh_bs_sim_", set, "_bs_", boot, ".mat"))[[1]]
...
sim = read_csv("./misc/ci_ex/sim_sep_9562.csv")
...
for (boot in 1:bootstraps) {
  load(paste0("./misc/nmf_ci_out/nmf_bs_sim_", set, "_bs_", boot, ".RDA"))
...
# EWA ####
plot_true_score  = scores_scaled[2,2]
plot_wa_v_dist   = vci_ewa_dist[2,2,]
plot_wa_bs_dist  = bs_ewa[2,2,]
plot_wa_nmf_dist = nmf_bs_score[2,2,]
...
plot_v_ewa   = vci_ewa[2,2]
plot_v_wa_25 = vci_ewa_lower[2,2]
plot_v_wa_75 = vci_ewa_upper[2,2]
...
# EH ####
plot_true_patterns = patterns_scaled[4,3]
plot_h_v_dist      = vci_eh_dist[4,3,]
plot_h_bs_dist     = bs_eh[4,3,]
plot_h_nmf_dist    = nmf_bs_eh[4,3,]
...
plot_v_eh   = vci_eh[4,3]
plot_v_h_25 = vci_eh_lower[4,3]
plot_v_h_75 = vci_eh_upper[4,3]
...
vci_diff = abs(chem - vci_pred)
nmf_diff = abs(chem - nmf_pred)
bs_diff = abs(chem - bs_pred_median)

...
create_scores_dist <- function (seed) {
  set.seed(seed)
  scores_dist <- rdirichlet(1000, c(rep(1, 4)))
...
# st dev of scores
apply(scores_dist_iter[1,3][[1]][[1]], 2, mean)
```
...
add_noise <- function (seed, chemx) {
  noise <- list(mean = 0, sd = apply(chemx, 2, sd)/10)
  set.seed(seed)
...
```{r} 
sim_normall <- rbind(sim_dist %>% mutate(data = "Distinct"),
...
sim_normall_1000 <- sim_normall %>% 
  filter(sim_factor == "1") %>% 
  mutate(sim = map(sim, function(x) x*1000))

# save(sim_normall_1000, file = "./Sims/sim_normall_1000.RDA")
# save(sim_normall, file = "./Sims/sim_normall.RDA")
```


```{r}
dist <- sim_dist$sim_1[[1]]*1000
apply(dist, 2, mean)
apply(dist, 2, sd)
```

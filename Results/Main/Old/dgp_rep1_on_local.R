#########################################################
# Data Generating Process Sims >> regular NMF (L2 & Poisson), PCA, and FA 
#########################################################
...
dgp_rep1 <- dgp_rep1 %>%
  mutate(pca_loadings_ssdist   = map2(true_patterns, pca_loadings,   symm_subspace_dist),
         ...
         nmfl2_loadings_ssdist = map2(true_patterns, nmfl2_loadings, symm_subspace_dist),
         nmfl2_scores_ssdist   = map2(true_scores,   nmfl2_scores,   symm_subspace_dist),
         nmfp_loadings_ssdist  = map2(true_patterns, nmfp_loadings,  symm_subspace_dist),
         nmfp_scores_ssdist    = map2(true_scores,   nmfp_scores,    symm_subspace_dist))

# Add BN2MF from MATLAB
...
dgp_rep1_all <- 
  ...
  mutate(bnmf_pred            = map2(ewa, eh, function(x,y) as.matrix(x) %*% as.matrix(y)),
         bnmf_loadings_ssdist  = map2(true_patterns, eh,  symm_subspace_dist),
         bnmf_scores_ssdist   = map2(true_scores,   ewa,    symm_subspace_dist),
         bnmf_rank = map(eh, nrow))

save(dgp_rep1_all, file = "./Results/Main/dgp_rep1_all.RDA")

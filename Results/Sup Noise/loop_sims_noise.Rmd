...
add_noise <- function (seed, chemx) {
  noise <- list(mean = 0, var = apply(chemx, 2, sd)/2)
  set.seed(seed)
...
```{r}
apply(sim_dist$chem[[1]], 2, var)
apply(sim_dist$chem[[1]], 2, mean)

apply(sim_dist$sim[[1]], 2, var)/10
apply(sim_dist$chem[[1]], 2, sd)/2

apply(sim_dist$true_scores[[1]], 2, var)
apply(sim_dist$true_patterns[[1]], 2, sd)

round(cor(sim_dist$sim[[1]])[1:10,1:10],2)
...
apply(sim_noise$chem[[1]], 2, var)
apply(sim_noise$sim[[1]], 2, var)
round(cor(sim_noise$sim[[201]]), 2)[1:10, 1:10]
round(cor(sim_noise$chem[[1]]), 2)[1:10, 1:10]

# for (i in 1:nrow(sim_noise)) {
#   write_csv(as_tibble(sim_noise$sim[[i]]), paste0("./Sims/sim_noise_rep1_", i, ".csv"))
# }

# save(sim_noise, file = "./Results/Sup Noise/sim_noise.RDA")
# load(paste0("./Results/Sup Noise/sim_noise.RDA"))
sim_noise
chem <- sim_noise$chem[[1]] %>% as_tibble()
sim <- sim_noise$sim[[1]] %>% as_tibble()
patterns <- sim_noise$true_patterns[[1]] %>% as_tibble()
scores <- sim_noise$true_scores[[1]] %>% as_tibble()

# write_csv(chem, "./Results/Sup Noise/noise_chem1.csv")
# write_csv(sim, "./Results/Sup Noise/noise_sim1.csv")
# write_csv(patterns, "./Results/Sup Noise/noise_patterns1.csv")
# write_csv(scores, "./Results/Sup Noise/noise_scores1.csv")
```

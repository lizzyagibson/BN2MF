## Combine BN2MF with VCI, noise results in MATLAB
## Combine other methods, noise results in R
...
## Combine R tibbles from `noise_other_models.R`
## Model output from PCA, FA, and NMF
noise_r = tibble()
for (i in 1:300) {
  load(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_other_models_out/noise_out_", i, ".RDA"))
  noise_r = bind_rows(noise_r, noise_out)
  print(i)
}
  
# Aggregate BN2MF results from `noise_bnmf_vci.m`
noise_m = tibble() # to combine with noise_r
noise_vci = tibble() # to combine with bootsraps

for (j in 1:300) {

  bn2mf_loadings = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_eh_NOTscaled", j, ".mat"))[[1]] %>% 
    as_tibble(.) %>% nest(data = everything()) %>% rename(bn2mf_loadings = data)

  bn2mf_scores = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_ewa_NOTscaled", j, ".mat"))[[1]] %>% 
    as_tibble(.) %>% nest(data= everything()) %>% rename(bn2mf_scores = data)

  eh_scaled = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_eh_scaled", j, ".mat"))[[1]] %>% 
    as_tibble(.) %>% nest(data = everything()) %>% rename(eh_scaled = data)

  ewa_scaled = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_ewa_scaled", j, ".mat"))[[1]] %>% 
    as_tibble(.) %>% nest(data= everything()) %>% rename(ewa_scaled = data)

  upperWA = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_upperWA_", j, ".mat"))[[1]] %>% 
    as_tibble(.) %>% nest(data= everything()) %>% rename(upperWA = data)

  lowerWA = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_lowerWA_", j, ".mat"))[[1]] %>% 
    as_tibble(.) %>% nest(data= everything()) %>% rename(lowerWA = data)

  upperH = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_upperH_", j, ".mat"))[[1]] %>% 
    as_tibble(.) %>% nest(data= everything()) %>% rename(upperH = data)

  lowerH = readMat(paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_vci_out/noise_lowerH_", j, ".mat"))[[1]] %>% 
  ...
  noise_m   <- bind_rows(noise_m, m_out)
  noise_vci <- bind_rows(noise_vci, vci_out)
  print(j)
}
...
noise = bind_cols(noise_r, noise_m)

# Save all
save(noise, file = "/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_out.RDA")

# Save VCI
save(noise_vci, file = "/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/vci_noise_out.RDA")
...
rel_err_loadings <- noise_metrics %>%
...
cos_dist_loadings <- noise_metrics %>%
                     filter(object == "loadings") %>% 
...
save(metrics, file = "/ifs/scratch/msph/ehs/eag2186/npbnmf/noise/noise_metrics.RDA")
# `noise_metrics.RDA` go into `plot_noise.R`

...
load("./Results/Sup Noise/sim_noise.RDA")
true_patterns = sim_noise[1,]$true_patterns[[1]]
true_scores = sim_noise[1,]$true_scores[[1]]

# Load some descriptive stats from matlab
t0_grid <- readMat("./MATLAB/test_t0/noise_t0_grid_out.mat")[[1]] %>% as_tibble()
colnames(t0_grid) = c("t0", "rank", "score", "iter", "mean_diff", "mean_T", "mean_reg")

# Load solution matrices with no annealing
ewa_reg <- readMat("./MATLAB/test_t0/ewa_noise.mat")[[1]]
varwa_reg <- readMat("./MATLAB/test_t0/varwa_noise.mat")[[1]]
eh_reg <- readMat("./MATLAB/test_t0/eh_noise.mat")[[1]]
varh_reg <- readMat("./MATLAB/test_t0/varh_noise.mat")[[1]]

tnot = seq(0, 0.01, by = .0005)

# load solution matrices with annealing across t0 values
t0 <- tibble()
for (i in 1:length(tnot)) {

  ewa <- readMat(paste0("./MATLAB/test_t0/noise_ewa_t0", tnot[i], ".mat"))[[1]]
  ewa <- as_tibble(ewa) %>% nest(ewa = everything())
  
  varwa <- readMat(paste0("./MATLAB/test_t0/noise_varwa_t0", tnot[i], ".mat"))[[1]]
  varwa <- as_tibble(varwa) %>% nest(varwa = everything())
  
  eh <- readMat(paste0("./MATLAB/test_t0/noise_eh_t0", tnot[i], ".mat"))[[1]]
  eh <- as_tibble(eh) %>% nest(eh = everything())

  varh <- readMat(paste0("./MATLAB/test_t0/noise_varh_t0", tnot[i], ".mat"))[[1]]

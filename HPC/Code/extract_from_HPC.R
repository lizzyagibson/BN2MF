...
for (i in 1:1300) {
  if (file.exists(here::here(paste0("/HPC/out_comb/out_", i, ".RDA")))) { 
    ...
    comb_out <- rbind(comb_out, output_all)
}
...
comb_out %>% dplyr::select(seed, grep("_ssdist", colnames(.))) %>%
...
# save(comb_out, file = "./HPC/Rout/comb_out.RDA")
load("./HPC/Rout/comb_out.RDA")
comb_out
...
comb_out %>% dplyr::select(seed, sim_type, sim, grep("pred", colnames(.))) %>% 
...
comb_out_error <- comb_out %>% dplyr::select(seed, sim_type, sim, chem, grep("pred", colnames(.))) %>% 
  pivot_longer(pca_pred:bnmf_pred) %>% 
...
# save(comb_out_error, file = "./HPC/Rout/comb_out_error.RDA")
...
comb_out_ssdist <- comb_out %>% dplyr::select(seed, sim_type, grep("_ssdist", colnames(.))) %>% 
...
# save(comb_out_ssdist, file = "./HPC/Rout/comb_out_ssdist.RDA")

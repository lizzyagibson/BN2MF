...
output_all <- sim_norm[job_num,] %>% 
  mutate(pca_out       = map(chem, get_pca),
...
output_all <- output_all %>% 
  mutate(pca_uncenter_out       = map(chem, get_pca_uncenter),
...
output_all <- output_all %>%
  mutate(nmf_l2_out      = map(chem, get_nmf_l2),
...
output_all <- output_all %>% 
  mutate(nmf_p_out      = map(chem, get_nmf_p),
...
output_all <- output_all %>% 
              mutate(bnmf_out      = map(chem, get_bnmf),
...
output_all <- output_all %>% 
  mutate(pca_norm              = map2(chem, pca_pred,    function(x,y) norm(x-y, "F")/norm(x, "F")),
         pca_uncenter_norm     = map2(chem, pca_uncenter_pred,    function(x,y) norm(x-y, "F")/norm(x, "F")),
         fa_norm               = map2(chem, fa_pred,     function(x,y) norm(x-y, "F")/norm(x, "F")),
         nmf_l2_norm           = map2(chem, nmf_l2_pred, function(x,y) norm(x-y, "F")/norm(x, "F")),
         nmf_p_norm            = map2(chem, nmf_p_pred,  function(x,y) norm(x-y, "F")/norm(x, "F")),
         bnmf_norm             = map2(chem, bnmf_pred,   function(x,y) norm(x-y, "F")/norm(x, "F")),
...
save(output_all, file = paste0("/ifs/scratch/msph/ehs/eag2186/npbnmf/chem_out/chem_sims", job_num, ".RDA"))

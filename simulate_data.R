...
set.seed(1988)
simulated <- rlnorm.rplus(1000, meanlog = mean_log, varlog = var_log) %>% as.matrix() %>% as_tibble()
colnames(simulated) <- names(mn_data)

# Simulate with multivariate normal function exponentiated
set.seed(1988)
simulated2 <- exp(mvrnorm(n = 1000, mu = mean_log, Sigma = var_log)) %>% as.matrix() %>% as_tibble()
...
grid.arrange(original, sim1_plot, nrow = 1)

grid.arrange(original, sim2_plot, nrow = 1)
